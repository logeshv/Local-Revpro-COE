...
BEGIN
   ...
   BEGIN
      INSERT INTO RPRO_REP_G (ID,
           ...
           VALUES (l_rep_seq,--RPRO_REP_ID_S_1.NEXTVAL,
                   'Siemens Waterfall Report QTR',
                   'Siemens Waterfall Report QTR',
                   'Custom',
                   'RPRO_SIEMENS_REP_CUSTOM_PKG.siemens_waterfall_jpn_rep',
   ...
   BEGIN
      ...
      UPDATE RPRO_LABEL_G
       ...
       WHERE TBL_NAME = 'SIEM_JAPAN_WF';  /*Added where condition by Gokul*/
   END;
      ...
      LOOP
         INSERT INTO RPRO_REP_FIELD_G (ID,
            ...
            SELECT RPRO_REP_FIELD_ID_S_1.NEXTVAL,
                   ...
                   l_period_id
              FROM RPRO_LABEL_G
             WHERE COL_NAME = rec.COL_NAME AND TBL_NAME = 'SIEM_JAPAN_WF';
         ...
         UPDATE RPRO_REP_FIELD_G
            SET indicators =
                   RPRO_REP_FIELD_PKG.SET_REQUIRED_FILTER (indicators,
                                                                  'Y')
          WHERE ALIAS = 'SIEM_WF_JPN' AND fld_name IN ('PERIOD_NAME','CURRENCY_TYPE','SEC_ATR_VAL'); --Added by Gokul
         
         UPDATE RPRO_REP_FIELD_G
            SET type =
                   'LOV'
          WHERE ALIAS = 'SIEM_WF_JPN' AND fld_name ='SEC_ATR_VAL'; /*Give the column name need to added as filter*/
          
         UPDATE RPRO_REP_FIELD_G
            SET indicators =
                   RPRO_REP_FIELD_PKG.set_filter_flag (indicators,
                                                              'Y')
          WHERE ALIAS = 'SIEM_WF_JPN' AND fld_name IN ('PERIOD_NAME','CURRENCY_TYPE','SEC_ATR_VAL'); /*Give the column name need to added as filter*/
      ...
      L_CNT:=L_CNT+1;
         INSERT INTO RPRO_LAYOUT_FIELD_G (ID,
            ...
            SELECT RPRO_LAYOUT_FIELD_ID_S_1.NEXTVAL,
              ...
              FROM RPRO_REP_FIELD_G
             WHERE FLD_NAME = i.FLD_NAME AND ALIAS = 'SIEM_WF_JPN' AND  rep_id = l_rep_seq;
         COMMIT;
      END LOOP;
   ...
   INSERT INTO RPRO_REP_FILTER_G (ID,
    ...
    SELECT RPRO_REP_FILTER_ID_S_1.NEXTVAL,
     ...
     WHERE RPRO_REP_FIELD_PKG.get_filter_flag (indicators) =
              'Y'
           AND ALIAS = 'SIEM_WF_JPN'--Added by gokul

CREATE OR REPLACE PACKAGE BODY RPRO_SIEMENS_CUSTOM_PKG
AS
   /*===================================================================================================+
   |                              ZUORA, San Jose, California                                           |
   +====================================================================================================+
   |                                                                                                    |
   |    File Name:         RPRO_SIEMENS_CUSTOM_PKG.pkb                                                  |
   |    Object Name:       RPRO_SIEMENS_CUSTOM_PKG                                                      |
   |                                                                                                    |
   |    Pre-reqs:                                                                                       |
   |                                                                                                    |
   |                                                                                                    |
   ...
   ---  Global Variables ---
   -------------------------
   gc_module         CONSTANT  VARCHAR2(150)     := 'RPRO_SIEMENS_CUSTOM_PKG';
  ...
  BEGIN
           ...
           LOOP
              ...
              FOR i IN 1 .. l_rcline.COUNT LOOP
                 ...
                 create_ooh_schedules( p_ooh_data       => l_rcline(i)
                                     , p_ooh_schd_data  => rpro_siemens_custom_pkg.g_ooh_schd_data
                                     , p_ooh_indx_tab   => rpro_siemens_custom_pkg.g_ooh_indx_tab
     ...
     THEN
        ...
        l_sql_stmt:=      'BEGIN
                              FORALL dat IN VALUES OF rpro_siemens_custom_pkg.g_ooh_indx_tab
                                 INSERT INTO '||g_tab_name||'
                                 VALUES  rpro_siemens_custom_pkg.g_ooh_schd_data(dat);
                           END;';
     ...
     l_sql_stmt := NULL;
     rpro_siemens_custom_pkg.g_ooh_indx_tab.DELETE;  --20190729
     rpro_siemens_custom_pkg.g_ooh_schd_data.DELETE; --20190729
     COMMIT;
  EXCEPTION WHEN OTHERS
  THEN
  ...
  -----------------------------------------------------------------------------
  PROCEDURE siemens_waterfall_rep ( p_report_id       IN     NUMBER
    ...
    l_proc                           VARCHAR2 (30) := 'SIEMENS_WATERFALL_REP';
  ...
  --ELSE   -- commented due to multi org changes 20201207
    ...
    /*Creating Snapshot for the given period*/
    l_stmt2           := q'(SELECT DECODE(cust.atr3,NULL,'REV','NO','NO','OOH','OOH') "Rev Type",siem_wf.f_ex_rate "F Ex Rate",siem_wf.g_ex_rate "G Ex Rate",)';--                                                                                                                                  --20140530
    ...
    l_amt_col  := CASE WHEN l_currency_type = 'F'
                       THEN ' cust.amount * siem_wf.f_ex_rate '
                       WHEN l_currency_type = 'R'
                       THEN ' cust.amount * siem_wf.g_ex_rate * siem_wf.f_ex_rate '
                       ELSE ' cust.amount '
                  END;
    ...
    l_stmt2 := l_stmt2 ||   '(SELECT NVL(SUM(amount),0)
                              ...
                              AND prd_id          <)'||l_run_prd_id||'
                              ) "Prior Amt" ,
                             cust.prd_id prd_id,
                             ---cust.amount amount
                             '||l_amt_col || ' amount
                           FROM '||g_tab_name||' cust,
                                 rpro_rc_line_g siem_wf
                           WHERE siem_wf.id                      = cust.line_id
                           )';
    l_head_value := l_head_value||'"Prior Amt",';
    ---l_top_stmt := l_top_stmt||l_stmt2||'PIVOT ( SUM( amount'||l_curr_rate||') FOR prd_id IN (';
    l_top_stmt1 := l_stmt2||'PIVOT ( SUM( amount) FOR prd_id IN (';

    FOR I IN (SELECT id,period_name FROM rpro_calendar_g WHERE id >=l_run_prd_id AND id<=NVL(l_end_prd_id,l_run_prd_id)) --MS20201207
    LOOP
       IF l_output_fmt = 'JSON'
       THEN
          p_header := p_header ||',{'
                             ||' "dataField":"'||i.period_name||'",'
                             ||RPRO_RP_FRMWRK_PKG.GET_JSON_HDR_TYPE('AMOUNT')
                             ||l_alignment
                             ||' "caption":"'||i.period_name||'"'
                             ||'}';
       ELSE
          p_header:=p_header||i.period_name||':';
       END IF;
       l_pivot_stmt:=l_pivot_stmt||i.id||'"'||i.period_name||'"'||',';
       l_head_value:=l_head_value||'NVL("'||i.period_name||'",0) "'||i.period_name||'",';
    END LOOP;
  ...
  THEN
     rpro_utility_pkg.record_log_act (l_proc
                                     ,'Main Exception: siemens_waterfall_rep'||SQLERRM||CHR(13)||dbms_utility.format_error_backtrace
                                     ,10
                                     );
  END siemens_waterfall_rep;
